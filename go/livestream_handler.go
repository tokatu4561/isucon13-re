...
func searchLivestreamsHandler(c echo.Context) error {
	...
	if c.QueryParam("tag") != "" {
		...
		query, params, err = sqlx.In("SELECT * FROM livestreams WHERE id IN (?) ORDER BY FIELD(id, ?)", livestreamIDs, livestreamIDs)
		if err != nil {
			return echo.NewHTTPError(http.StatusInternalServerError, "failed to construct IN query: "+err.Error())
		}
...
func fillLivestreamResponse(ctx context.Context, tx *sqlx.Tx, livestreamModel LivestreamModel) (Livestream, error) {
	...
	gTagModels, _ := getTags()
	for i := range livestreamTagModels {
		tagModel := TagModel{}
		for _, gTagModel := range gTagModels {
			if gTagModel.ID == livestreamTagModels[i].TagID {
				tagModel = *gTagModel
				break
			}
		}
